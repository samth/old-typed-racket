...
(require "datatype-printer-props.ss"
         ...
         (lib "plt-match.ss")
         (lib "simplify-patterns.ss" "mzlib" "private" "match"))
(require-for-syntax
         (lib "simplify-patterns.ss" "mzlib" "private" "match"))
...
(define-syntax (define-type-internal stx)
  (syntax-case stx ()
    [(_ nm top-pred [variant maker (fld ty) ...] ...)
     #'(begin 
         (#%app void (quote-syntax (define-typed-struct-internal variant ([fld : ty] ...) #:maker maker))) ...
         (#%app void (quote-syntax (define-type-alias-internal nm (Un variant ...))))
         (#%app void (quote-syntax (assert-predicate-internal nm top-pred))))]))
             ...
             (mz:begin
              ...
              (define-match-expander type:
                (lambda (stx)
                  (syntax-case stx ()
                    [(_ arg)
                     (let loop ([cnt (match-...-nesting)] [t 'type])
                       (if (zero? cnt)
                           (syntax-property #'arg 'type-label t)
                           (loop (sub1 cnt) `(Listof ,t))))])))
              (define-type-internal type %type? [variant %variant (field pred?) ...] ...))))))]))
