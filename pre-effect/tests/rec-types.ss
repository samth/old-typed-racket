...
(module rec-types (planet "typed-scheme.ss" ("plt" "typed-scheme.plt"))
  ...
  (pdefine: (a) (heap-size [h : (Heap a)]) : number
            (cond [(heap-empty? h) 0]
                  [(heap-node? h)                  
                   (+ 1 (+ (heap-size #{(heap-node-left #{h : (heap-node a)}) : (Heap a)})
                           (heap-size #{(heap-node-right #{h : (heap-node a)}) : (Heap a)})))]
