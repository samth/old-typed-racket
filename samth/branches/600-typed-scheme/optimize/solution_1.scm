...
(require (only-in "../optimize/csu660.ss" number: match symbol: string->sexpr test second first rest))
         
(define-type-alias Sexpr (mu x (U Symbol String Number Boolean (Listof x))))
...
(define-type TOY
  ...
  [BindRec (names Names)
           (exprs (list-of TOY))
           (body (list-of TOY))]
  [Fun  (names Names) (body (list-of TOY))]
  [RFun (names Names) (body (list-of TOY))]
...
(define: (parse-sexpr [sexpr : Sexpr]) : TOY
  (match sexpr
    [(number: n)    (Num n)]
    [(symbol: name) (Id name)]    
    [(cons (or 'bind 'bindrec) more)
     (match sexpr
       [(list binder (list (list (symbol: #{names : (Listof Symbol)}) #{nameds : (Listof Sexpr)}) ...) #{body : (Listof Sexpr)} ...)
    ...
    [(cons (or 'fun 'rfun) more)
     (match sexpr
       [(list funer (list (symbol: #{names : (Listof Symbol)}) ...) #{body : (Listof Sexpr)} ...)
    ...
    [(list fun #{args : (Listof Sexpr)} ...) ; other lists are applications
...
(define-type VAL
  ...
  [ScmV   (x Any)]
  [FunV   (names Names) (body (list-of TOY)) (env ENV)]
  [RFunV  (names Names) (body (list-of TOY)) (env ENV)]
  [PrimV  (prim ((Listof VAL) -> VAL))])

;; optimize: a bogus value
(define: bogus : VAL (BogusV))

;; a frame is an association list of names and values.
(define-type-alias Frame (Listof (Tuple Symbol (Box VAL))))
#;
(define frame?
  (list-of (lambda (x)
             (and (list? x)
...
(define: (extend-boxes [names : Names] [boxes : (Listof (Box VAL))] [env : ENV]) : ENV
  (if (= (length names) (length boxes))
      ;; CHANGE MADE HERE
    (FrameEnv (map (lambda: ([a : Symbol] [b : (Box VAL)]) (list a b)) names boxes) env)
...
(define: (extend [names : Names] [values : (Listof VAL)] [env : ENV]) : ENV
  (extend-boxes names (map (inst box : VAL) values) env))

;; extend-rec : (list-of symbol) (list-of TOY) ENV -> ENV
;; extends an environment with a new recursive frame.
(define: (extend-rec [names : Names] [exprs : (Listof TOY)] [env : ENV]) : ENV
  (if (= (length names) (length exprs))
    (let* ([new-cells (map (lambda: ([x : Any]) (box bogus)) exprs)]
           [new-env (FrameEnv (map (lambda: ([a : Symbol] [b : (Box VAL)]) (list a b)) names new-cells) env)]
           [values (map (lambda: ([e : TOY]) (eval e new-env)) exprs)])
      (for-each (lambda: ([cell : (Box VAL)] [val : VAL]) (set-box! cell val))
                new-cells values)
      new-env)
...
(define: (lookup [name : Symbol] [env : ENV]) : (Box VAL)
  (cases env
...
(define: global-environment : ENV
  (FrameEnv (list #|(list '+ (scheme-func->prim-val +))
                  ...
                  ;; values
                  (list 'true  (box (ann (ScmV #t) : VAL)))
                  (list 'false (box (ann (ScmV #f) : VAL))))
            (EmptyEnv)))
...
(define: (eval-body [exprs : (Listof TOY)] [env : ENV]) : VAL
  (if (null? exprs)
...
(define: (eval [expr : TOY] [env : ENV]) : VAL
  (cases expr
    [(Num n) (ScmV n)]
    ...
    [(Bind names exprs bound-body)
     (eval-body bound-body
                (extend names
                        (map (lambda: ([e : TOY]) (eval e env)) exprs)
                        env))]
    ...
    [(Call fun-expr arg-exprs)
     (let ([fval (eval fun-expr env)]
           ...
           [arg-vals (lambda ()
                       (map (lambda: ([e : TOY]) (eval e env)) arg-exprs))])
       (cases fval
         ...
         [(RFunV names body fun-env)
          (if (andmap Id? arg-exprs)
            (let ([boxes (map (lambda: ([id : Id]) (lookup (Id-name id) env))
                              arg-exprs)])
