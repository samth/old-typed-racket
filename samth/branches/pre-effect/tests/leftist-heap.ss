...
(module leftist-heap (planet "typed-scheme.ss" ("plt" "typed-scheme.plt"))
;(module leftist-heap mzscheme
  (require 
   (all-except (lib "67.ss" "srfi") current-compare =? <?)
   "typed-list.ss"
   #;(lib "42.ss" "srfi")
  ...
  ;; FIXME
  (define: empty : (All (a) (case-lambda (-> (Heap a)) (comparator -> (Heap a))))
    (pcase-lambda: (a)
      [()    #{(make-heap-empty (current-compare)) : (Heap a)}]
      [([cmp : comparator]) #{(make-heap-empty cmp) : (Heap a)}]))
  
  (define: empty? : (pred heap-empty) heap-empty?)
  
  ;; added extra case because remove not working correctly
  ;; added annotation on h
  (pdefine: (a) (rank [h : (Heap a)]) : number
    (if (empty? h)
        0
        (if (heap-node? h)
            (heap-node-rank #{h : (heap-node a)})
            (error "Never happens"))))
  
  (pdefine: (a) (make [x : a] [a : (Heap a)] [b : (Heap a)]) : (Heap a) 
    (let ([ra (rank a)] 
          [rb (rank b)]
          [cmp (heap-compare #{a : heap})])
      (if (>= ra rb)
  ...
  (pdefine: (a) (union [h1 : (Heap a)] [h2 : (Heap a)]) : (Heap a)
    (cond
      ...
      [else        (let*: (;; added new bindings at simpler types
                           [h1 : (heap-node a) h1] 
                           [h2 : (heap-node a) h2] 
                           [x : a (heap-node-elm h1)]
                           [y : a (heap-node-elm h2)])
                          (if<=? ((heap-compare h1) x y)
                                (make x (heap-node-left h1) (union #{(heap-node-right h1) : (Heap a)} #{h2 : (Heap a)}))
                                (make y (heap-node-left h2) (union #{h1 : (Heap a)} #{(heap-node-right h2) : (Heap a)}))))]))
 
  (pdefine: (a) (insert [x : a] [h : (Heap a)]) : (Heap a)
    (let: ([cmp : comparator (heap-compare h)])
      (union #{(make-heap-node cmp #{1 : number} x #{(make-heap-empty cmp) : (Heap a)} #{(make-heap-empty cmp) : (Heap a)}) : (Heap a)}
             h)))
  ...
  (pdefine: (a) (elements [h : (Heap a)]) : (list-of a)
    (fold (lambda: ([x : a] [l : (list-of a)]) (cons x l))
          #;#{cons : (a (list-of a) -> (list-of a))} #{'() :: (list-of a)} h))
         ...
         (define: (merge-pairs [hs : (list-of (Heap a))]) : (list-of (Heap a))
           (cond
             [(or (null? hs)
                  (null? (cdr hs))) hs]
             [else 
              (cons #{(union (car hs) (cadr hs)) :: (Heap a)}
                    #{(merge-pairs (cddr hs)) :: (list-of (Heap a))})]))
         (if (null? hs)
             e
  ...
  (define: singleton : (All (a) (case-lambda (a -> (Heap a)) (comparator a -> (Heap a))))
    (pcase-lambda: (a)
      [([x : a])     (insert x (empty))]
      [([cmp : comparator] [x : a]) (insert x #{(make-heap-empty cmp) : (Heap a)})]))
