(module subtype-tests mzscheme
  ...
  (require/private subtype types-aux types infer planet-requires)
  
  (require-libs)
  
  (require-schemeunit)
  
  (provide (all-defined-except go go/gui))
  ...
  (define (subtype-tests)
    ...
    #| [(-mu s (-> N s)) (-mu t (-> N (-> N t)))]
     ...
     [(-poly (a) (make-Listof (make-tvar 'a))) (make-Listof N)]     ;; 
     [(-poly (a) N) N]
     ...
     [(-poly (a) (make-Listof (make-tvar 'a))) (make-Listof (-mu x (Un (make-Listof x) N)))]
     ...
     [(-poly (a) ((Un (make-base-type 'foo) (-struct 'bar #f (list N a) (list))) . -> . (-lst a)))
      ((Un (make-base-type 'foo) (-struct 'bar #f (list N (-pair N (-v a))) (list))) . -> . (-lst (-pair N (-v a))))]
     [(-poly (a) ((-struct 'bar #f (list N a) (list)) . -> . (-lst a)))
      ((-struct 'bar #f (list N (-pair N (-v a))) (list)) . -> . (-lst (-pair N (-v a))))]
      
     [(-poly (a) (a . -> . (make-Listof a))) ((-v b) . -> . (make-Listof (-v b)))]
     [(-poly (a) (a . -> . (make-Listof a))) ((make-pair-ty N (-v b)) . -> . (make-Listof (make-pair-ty N (-v b))))]
|#
     ))
