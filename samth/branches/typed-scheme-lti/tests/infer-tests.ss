(module infer-tests mzscheme
  ...
  #;(require/expose (file "../private/infer2.ss") (combine table:un exn:infer?))
  (provide fv-tests i2-tests combine-tests)
  
  (define-simple-syntax (fv-t ty elems ...)
    (let ([ty* ty])
  ...
  (define-simple-syntax (i2-t t1 t2 (a b) ...)
    (test-check (format "~a ~a" t1 t2)
                equal?
                (infer t1 t2 (fv t1))
                (list (list a b) ...)))
  
  (define-simple-syntax (i2-l t1 t2 fv (a b) ...)
    (test-check (format "~a ~a" t1 t2)
                equal?
                (infer/list t1 t2 fv)
                (list (list a b) ...)))
  ...
  (define-simple-syntax (i2-f t1 t2)
